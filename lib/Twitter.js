...
Twitter.prototype.createTweet = function (body, error, success) {
    var path = '/statuses/update.json';
    var url = this.baseUrl + path;
    this.postRequest(url, body, 'application/json', error, success);
};
...
Twitter.prototype.postRequest = function (url, post_body, post_content_type, error, success) {
    this.oauth.post(url, this.accessToken, this.accessTokenSecret,  post_body, post_content_type, function (err, body, response) {
        console.log('URL [%s]', url);
        console.log('response ' + response.statusCode);
        if (!err && [200, 201, 202].indexOf(response.statusCode)) {
            success(body);
        } else {
            error(err, response, body);
        }
